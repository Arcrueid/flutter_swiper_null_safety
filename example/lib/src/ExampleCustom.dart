...
class _ExampleCustomState extends State<ExampleCustom> {
  ...
  void didUpdateWidget(ExampleCustom oldWidget) {
    customLayoutOption = CustomLayoutOption(startIndex: -1, stateCount: 3).addRotate(
        [-45.0 / 180, 0.0, 45.0 / 180]).addTranslate([Offset(-370.0, -40.0), Offset(0.0, 0.0), Offset(370.0, -40.0)]);
  ...
  void initState() {
    customLayoutOption = CustomLayoutOption(startIndex: -1, stateCount: 3).addRotate(
        [-25.0 / 180, 0.0, 25.0 / 180]).addTranslate([Offset(-350.0, 0.0), Offset(0.0, 0.0), Offset(350.0, 0.0)]);
    _fade = 1.0;
    _currentIndex = 0;
    _curve = Curves.ease;
    _scale = 0.8;
...
// maintain the index
  ...
  Widget buildSwiper() {
    return Swiper(
      ...
      pagination:
          SwiperPagination(builder: const DotSwiperPaginationBuilder(size: 20.0, activeSize: 20.0, space: 10.0)),
    );
  }
  ...
  Widget build(BuildContext context) {
    return Column(children: <Widget>[
      Container(
        ...
        child: SizedBox(height: 300.0, width: double.infinity, child: buildSwiper()),
      ),
      Expanded(
          child: ListView(
        children: <Widget>[
          ...
          FormWidget(
            label: 'loop',
            child: Switch(value: _loop, onChanged: (bool value) => setState(() => _loop = value)),
          ),
          FormWidget(
            label: 'outer',
            child: Switch(value: _outer, onChanged: (bool value) => setState(() => _outer = value)),
          ),
          //Pannel Begin
          FormWidget(
            label: 'autoplay',
            child: Switch(value: _autoplay, onChanged: (bool value) => setState(() => _autoplay = value)),
          ),
